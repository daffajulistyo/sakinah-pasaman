...
class MasterKegiatanController extends Controller
{
    public function create(Request $request)
    {
        try {
            ...
            $data = MasterKegiatan::create($form);

            return response()->json([
                'success' => true,
                'message' => 'Successfully created new kegiatan.',
                'data' => $data,
            ]);
        } catch (\Throwable $th) {
            return response()->json([
    ...
    public function read($id)
    {
        try {
            if (!Str::isUuid($id)) {
                return response()->json([
                    'success' => false,
                    'message' => 'Invalid Id, Kegiatan not Found',
                ], 422);
            }
            $kegiatan = MasterKegiatan::with('program.opd')->find($id);
            if (!$kegiatan) {
                return response()->json([
                    'success' => false,
    ...
    public function update($id, Request $request)
    {
        try {
            ...
            $kegiatan = MasterKegiatan::find($id);
            if (!$kegiatan) {
                return response()->json([
                    'success' => false,
                    'message' => 'Kegiatan not found.',
    ...
    {
        $searchColumn = collect(['kode_kegiatan', 'nama_kegiatan']);
        ...
        $query = MasterKegiatan::with('program.opd');
        ...
        $objData = $objData->map(function ($item) {
            ...
            return [
                ...
                "master_program_id" => $item->master_program_id,
                "nama_program"      => $item->program->nama_program ?? null,
                "nama_opd"          => $item->program->opd->nama_opd ?? null,
        ...
        return response()->json([
            'success' => true,
            'message' => 'List of Master Data Kegiatan',
            'data' => $objData,
            'pagination' => [
                'page' => $currentPage,
                'per_page' => $perPage,
