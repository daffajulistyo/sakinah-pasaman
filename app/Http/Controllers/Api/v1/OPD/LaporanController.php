...
class LaporanController extends Controller
{
         ...
         try {
            ...
            if (!$opd) {
             ...
             $tujuan = $tujuan->map(function($item) use($request, $tahun_awal) {
                ...
                $satuan = BaseController::getSatuanByID($it->satuan_id);
                ...
                $sasaran = SasaranOpd::where('tujuan_opd_id', '=', $item->id)->where('parent_id', '=', 0)
                                    ->where('is_active', true)
                                    ->orderBy('order', 'asc')
                                    ->get();
    ...
    public function cetak_data_kinerja(Request $request)
    {
         try {
            ...
            $pdf = Pdf::loadView('report_template.opd.data_kinerja', compact('data'))
                        ->setPaper('Legal', 'landscape');
            return $pdf->download('Kinerja_'.str_replace($opd->nama_opd,' ','_').'.pdf');
    ...
    public function update_realisasi($id, Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Realisasi Renstra updated successfully.',
                'data' => $indikator,
            ]);
        } catch (\Throwable $th) {
            return response()->json([
    ...
    public function capaian(Request $request)
    {
         try {
            ...
            if (!$opd) {
             ...
             $tujuan = TujuanOpd::where('pohon_kinerja_visi_id', '=', $visi->id)->where('master_opd_id', $master_opd_id)
                                    ->where('is_active', true)
                                    ->orderBy('order', 'asc')
                                    ->get();
