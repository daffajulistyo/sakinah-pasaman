...
class AdminKdhController extends Controller
{
    //
    /**
    ...
    {
        //
        return view('app.data.adminkdh._index');
    }

    public function update()
    {
        //
        return view('app.data.adminkdh._tambah');
    }
    ...
    public function store(Request $request)
    {
        ...
        $role_admin_kdh = "00f34f96-7cdf-4c9c-abf2-ea4fe1d24a08";
        $existing_admin_kdh = Roleplay::where('user_id', $request->user_id)->where('role_id', $role_admin_kdh)->first();
        if(!$existing_admin_kdh){
            $user = User::find($request->user_id);
            if($user){
                $attachRole = [
                    [
                        "id" => Str::uuid(),
                        "role_id" => $role_admin_kdh,
                        "type" => "common",
        ...
        return response()->json([
            'status' => true,
            'message' => 'Pegawai berhasil ditambahkan sebagai admin KDH',
        ]);
    ...
    public function destroy($user_id,$id_roleplay)
    {
        ...
        return response()->json([
            'message' => $name.' berhasil dihapus dari daftar admin KDH',
        ]);
    }
    ...
    public function datatable(Request $request)
    {
        ...
        $query = User::query()->select('a.name','a.username','b.created_at','a.id','us.id AS user_sakip_id', 'a.is_active', 'usg.opd_nm AS opd', 'usg.jabatan_nm AS jabatan', 'b.id AS roleplay_id', 'a.id AS user_id')
                ...
                ->where('b.role_id', '=', '00f34f96-7cdf-4c9c-abf2-ea4fe1d24a08'); // Admin_KDH
