...
@section('page-title', 'Master Sub Kegiatan')
@section('main-content')

<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h3 class="card-title mb-0">Master Sub Kegiatan</h3>
    ...
    <div class="card-body">
        <div class="mb-3">
            <form method="GET" class="row g-2">
                ...
                @if($search)<div class="col-auto"><a href="{{ route('sub-kegiatan.index') }}" class="btn btn-sm btn-outline-danger">&times; Reset</a></div>@endif
            </form>
        </div>
            ...
            <table class="table table-bordered table-hover mb-0 small">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $row->kegiatan->kode_kegiatan ?? '-' }} - {{ $row->kegiatan->nama_kegiatan ?? '-' }}</td>
                        <td>{{ $row->kegiatan->program->kode_program ?? '-' }}</td>
                        ...
                        <td class="text-center">
                            <button class="btn btn-sm btn-outline-primary btn-sm-table" title="Edit"
                                ...
                                data-mode="edit"
                                data-id="{{ $row->id }}"
                                data-kode="{{ $row->kode_sub_kegiatan }}"
                                data-nama="{{ $row->nama_sub_kegiatan }}"
                                data-kegiatan="{{ $row->master_kegiatan_id }}"
                                data-anggaran="{{ $row->anggaran }}"
                                data-aktif="{{ $row->is_active }}"><i class="bi bi-pencil"></i></button>
                            <button class="btn btn-sm btn-outline-danger btn-sm-table" title="Hapus" data-bs-toggle="modal" data-bs-target="#deleteModal" data-action="{{ route('sub-kegiatan.destroy', $row->id) }}"><i class="bi bi-trash"></i></button>
                        </td>
                    </tr>
                    @empty
        ...
        <div class="modal-content">
            <form id="formModalForm" method="POST">
                ...
                <div class="modal-header">
                    <h6 class="modal-title" id="formModalTitle">Tambah Sub Kegiatan</h6>
                    <button class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <input type="hidden" name="_method" id="formMethod" value="POST">
                    <div class="mb-3">
                        <label for="kode_sub_kegiatan" class="form-label">Kode Sub Kegiatan</label>
                        <input type="text" name="kode_sub_kegiatan" id="kode_sub_kegiatan" class="form-control form-control-sm" required>
                    </div>
                    <div class="mb-3">
                        <label for="nama_sub_kegiatan" class="form-label">Nama Sub Kegiatan</label>
                        <input type="text" name="nama_sub_kegiatan" id="nama_sub_kegiatan" class="form-control form-control-sm" required>
                    </div>
                    <div class="mb-3">
                        <label for="master_kegiatan_id" class="form-label">Kegiatan</label>
                        <select name="master_kegiatan_id" id="master_kegiatan_id" class="form-select form-select-sm" required>
                            <option value="">-- Pilih Kegiatan --</option>
                            @foreach($kegiatans as $k)
                            <option value="{{ $k->id }}">{{ $k->kode_kegiatan }} - {{ $k->nama_kegiatan }} ({{ $k->program->kode_program ?? '-' }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    formModal.addEventListener('show.bs.modal', function(e) {
        ...
        if (mode === 'edit') {
            title.textContent = 'Edit Sub Kegiatan';
            frm.action = '{{ url("backend/sub-kegiatan") }}/' + btn.getAttribute('data-id');
            method.value = 'PUT';
            document.getElementById('kode_sub_kegiatan').value = btn.getAttribute('data-kode');
            document.getElementById('nama_sub_kegiatan').value = btn.getAttribute('data-nama');
            document.getElementById('master_kegiatan_id').value = btn.getAttribute('data-kegiatan');
            ...
            title.textContent = 'Tambah Sub Kegiatan';
            frm.action = '{{ route("sub-kegiatan.store") }}';
            method.value = 'POST';
            frm.reset();
