                    ...
                    @if($node['indikator']->count() > 0)
                        ...
                        @foreach($node['indikator'] as $di)
                         ...
                         @php $x++; @endphp
                                ...
                                <td> {{ !empty($di['target_tw1']->target_tw1) ? $di['target_tw1']->target_tw1 : '' }}</td>
                                <td> {{ !empty($di['target_tw2']->target_tw1) ? $di['target_tw1']->target_tw2 : '' }}</td>
                                <td> {{ !empty($di['target_tw3']->target_tw1) ? $di['target_tw1']->target_tw3 : '' }}</td>
                                <td> {{ !empty($di['target_tw4']->target_tw1) ? $di['target_tw1']->target_tw4 : '' }}</td>
                                <td>
                                    <ol>
                                    @foreach($di['langkah'] as $dl)
                                        <li> {{ $dl['langkah'] }}</li>
                                    @endforeach
                                    </ol>
                                </td>
                                <td rowspan="{{ $node['indikator']->count() }}">
                                    {{ !empty($di['anggaran_perjanjian_kinerja']->murni) ? $di['anggaran_perjanjian_kinerja']->murni : '' }}
                                </td>
                                @else
                                <tr>
                                    <td>{{ $x }} {{ $di['indikator'] }}</td>
