...
<body>
    <h2 style="text-align:center;"> RENSTRA</h2>
    ...
    <table class="report">
        <thead>           
            ...
            <tr>                
                    <td colspan="3"> {{ $data['visi']->period_starts + 1; }} </td>
                    <td colspan="3"> {{ $data['visi']->period_starts + 2; }} </td>
                    <td colspan="3"> {{ $data['visi']->period_starts + 3; }} </td>
                    <td colspan="3"> {{ $data['visi']->period_starts + 4; }} </td>
                    <td colspan="3"> {{ $data['visi']->period_starts + 5; }} </td>
            </tr>
        ...
        <tbody>
         ...
         @if($data['tujuan']->count() > 0)
            ...
            @php $a++;  @endphp
                <tr>
                    <td colspan="19">{{ $a .'. '.$tujuan['tujuan'] }}</td>
                </tr>
                @if($tujuan['sasaran']->count() > 0)
                @php $i = 1;  @endphp
                @foreach($tujuan['sasaran'] as $node)
                    <tr>
                        <td rowspan="{{ $node['indikator_sasaran']->count() }}">&nbsp;</td>
                        <td rowspan="{{ $node['indikator_sasaran']->count() }}">{{ $i.'. '.$node['sasaran'] }}</td>
                    ...
                    @php $i++; @endphp
                        @if($node['indikator_sasaran']->count() > 0)
                            ...
                            @php $x++; @endphp
                                    ...
                                    <td> {{ !empty($di['satuan'])   ? $di['satuan'] : '' }}</td>
