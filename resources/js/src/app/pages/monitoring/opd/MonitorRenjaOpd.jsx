...
import { getListMonitorRenjaOpd } from '@/redux/ducks/monitoring/action'
...
const RenjaForm = () => {
    ...
    const getDataTable = async (year, period) => {
        ...
        const response = await dispatch(getListMonitorRenjaOpd({tahun, murni, master_opd_id: idopd}))
    }
    const tableHeader = () => (
        <tr>
    ...
    const renderTable = () => {
        if(selectedYear !== "" && selectedPeriod !== null) {
            return monitoringState.data_renja.length > 0 ? monitoringState.data_renja.map((item, x) => (
                <>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 px-3 w-full">
                <div className="dark:text-white">
                    <div className="flex flex-row items-center gap-3">
                        ...
                        <div className="lg:text-lg font-bold text-teal-500 dark:text-white">RENJA Perangkat Daerah</div>
                    </div>
                </div>
            </div>
