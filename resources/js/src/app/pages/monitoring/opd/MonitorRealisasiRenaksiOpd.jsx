...
import { getListMonitorRealisasiRenaksiOpd } from '@/redux/ducks/monitoring/action'
import axios from 'axios'


const MonitorRealisasiRenaksiOpd = () => {
    ...
    const getDataTable = (year = "") => {
        dispatch(getListMonitorRealisasiRenaksiOpd({ tahun: year != "" ? year : selectedYear, master_opd_id: idopd }))        
    }
    ...
    React.useEffect(() => {
        initFlowbite()
    },[monitoringState.data_realisasirenaksi])
    const tableHeader = () => (
        <>
            <tr>
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/monitoring/opd/realisasi/cetak?master_opd_id=${idopd}&tahun=${selectedYear}`
            ...
            let filename = `Realisasi_Renaksi_Perangkat_Daerah_${namaOpd}.pdf`; //    fallback filename
            if (disposition) {
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                ...
                <div className="block w-full p-4">
                    <StaticTable header={tableHeader()}>
                    {
                        ...
                        monitoringState.data_realisasirenaksi.length === 0 ?
                        <tr className="border-b dark:border-gray-700">
                            <td className="px-4 py-3 border text-center" colSpan="100%">( no data )</td>
                        </tr> :
                        monitoringState.data_realisasirenaksi.map((item, key) => (
                            <>
...
export default MonitorRealisasiRenaksiOpd