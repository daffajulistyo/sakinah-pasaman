...
import { getListRenaksiKdhLangkah, createRenaksiKdhLangkah, updateRenaksiKdhLangkah, deleteRenaksiKdhLangkah, getListRenaksiKdh } from '@/redux/ducks/ranaksikdh/action'
...
const LangkahRa = () => {
    ...
    const getDataTable = () => {
        dispatch(getListRenaksiKdhLangkah({
    ...
    React.useEffect(() => {
        if(renaksiKdhState.list.length === 0){ 
            dispatch(getListRenaksiKdh({ tahun: yearActive }))
        }
        else{
            let baseData = null
            let dataSasaran = renaksiKdhState.list.find((item) => {
    ...
    React.useEffect(() => { initFlowbite() }, [renaksiKdhState.list_langkah])
    ...
    const renderTable = () => (
        renaksiKdhState.list_langkah.length > 0 ? renaksiKdhState.list_langkah.map((item, x) => (
    ...
    const simpanData= async ()=> {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const payload = {
                ...form,
                tahun: yearActive,
                pohon_kinerja_sasaran_id: sasaranidActive,
                pohon_kinerja_indikator_id: indikatoridActive,
            }
    ...
    const deleteAction = (id) => {
        ...
        confirmButtonText: 'Yes!'
            ...
            if (result.isConfirmed) {
                const response = await dispatch(deleteRenaksiKdhLangkah(id))
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    ...
                    <div className="flex justify-between gap-3">
                        <PrimaryLinkBtn to={`/perencanaan/kdh/ra`}>
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        !renaksiKdhState.loading ? renderTable() :
                        <tr className="border-b dark:border-gray-700">
