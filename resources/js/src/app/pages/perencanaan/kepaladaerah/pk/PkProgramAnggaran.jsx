...
import { createProgramPkKdh, getListPkKdhProgram } from '@/redux/ducks/pkkdh/action'
import TablePA from '@/app/components/ProgramAnggaran/TableProgramAnggaran'
import Swal from 'sweetalert2'

const PkProgramAnggaran = () => {
    ...
    React.useEffect(() => {
        ...
        dispatch(getListPkKdhProgram(payload))
    },[])
    const [programExisted,setProgramExisted] = React.useState(null)
    React.useEffect(() => {
        if(pkKdhState.sasaran !== null){
            if(pkKdhState.sasaran.program_rkpd){
                if(pkKdhState.sasaran.program_rkpd.list_program){
                    var existedProgram = pkKdhState.sasaran.program_rkpd.list_program
    ...
    const opdOptions = () => {
        let opdPendukung = pkKdhState.sasaran?.opd_pendukung ?? null
        if(opdPendukung){
    ...
    const mixedLoading = () => (programAnggaranState.loading || pkKdhState.loading)
        ...
        Swal.fire({
            ...
            confirmButtonText: 'Yes!'
                ...
                if (result.isConfirmed) {
                    const response = await dispatch(createProgramPkKdh(payload))
                    ...
                    if(response.error === null){
                        ...
                        navigate(`/perencanaan/kdh/pk/${period}/${type}`)
                    }
                    else{
                        Swal.fire({
                            icon: 'error',
        ...
        <Layout>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    ...
                    <div className="flex">
                        <PrimaryLinkBtn to={`/perencanaan/kdh/pk/${period}/${type}`}>
                ...
                <div className="block w-full p-4">
                    ...
                    <MyInput
                        id="sasaranSelector"
                        label="Sasaran"
                        value={pkKdhState.sasaran?.sasaran?.sasaran ?? ""}
                        disabled
                    />
                    <MySelect2
                        id="opdSelector"
...
export default PkProgramAnggaran