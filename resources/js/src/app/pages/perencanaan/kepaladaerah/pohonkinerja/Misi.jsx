...
import { getListMisiKdh, createMisiKdh, updateMisiKdh, deleteMisiKdh } from '@/redux/ducks/misikdh/action'
...
import { getVisiKdh } from '@/redux/ducks/visikdh/action'
...
const Misi = () => {
    ...
    const misiKdhState = useSelector((state) => state.misiKdhState)
    const visiKdhState = useSelector((state) => state.visiKdhState)
    const dispatch = useDispatch()
    const [searchParams, setSearchParams] = useSearchParams()
    const [openModal, setOpenModal] = React.useState(false)
    const [formTitle, setFormTitle] = React.useState("FORM TAMBAH MISI")
    const [selectedVisi, setSelectedVisi] = React.useState("Visi")
    const [editId, setEditId] = React.useState("")
    const selectedVisiId = searchParams.get("visiId")
    const getDataTable = async (page = 1, per_page = 10, search = "") => {
        const response = await dispatch(getListMisiKdh({ page, per_page, search, pohon_kinerja_visi_id: selectedVisiId ?? "" }))
    ...
    React.useEffect(() => { initFlowbite() },[misiKdhState])
    React.useEffect(() => { visiKdhState.data ? setSelectedVisi(visiKdhState.data.visi) : null },[visiKdhState.data])
    React.useEffect(() => {
        
        selectedVisiId ?
        dispatch( getVisiKdh(selectedVisiId ?? "") ).then((result) => {
    ...
    const formik = useFormik({
        ...
        validationSchema: Yup.object({ 
            ...
            misi:           Yup.string().required().strict(true),
    ...
    const validationForm = async () => {
        ...
        formik.setFieldTouched('misi', true, true)
    ...
    const simpanData= async ()=> {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const payload = {
                pohon_kinerja_visi_id: selectedVisiId,
                order: form.order,
                misi: form.misi,
    ...
    const editAction = (data) => {
        ...
        formik.setFieldValue('misi', data.misi);
        formik.setFieldValue('isActive', data.is_active)

        setEditId(data.id)
        
        setFormTitle("FORM EDIT MISI")
        setOpenModal(true);
    }
    const deleteAction = (id) => {
        ...
        confirmButtonText: 'Yes!'
            ...
            if (result.isConfirmed) {
                const response = await dispatch(deleteMisiKdh(id))
    ...
    return (
        <Layout loading={visiKdhState.loading} >
            <HeaderPohonKinerja />
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-5 px-4 w-full flex min-h-[35rem]">
                <TabMenuPohonKinerja active='misi' params={{ visiId: selectedVisiId }} />
                <div className="flex flex-col w-full mt-14 p-3 border border-teal-500/30 dark:border-gray-600 rounded-lg">
                    <div className="w-full text-center text-lg text-teal-500 dark:text-white font-bold mb-3">MISI KEPALA DAERAH</div>
                    <div className="w-full text-center text-lg dark:text-white italic mb-3">Visi : " {selectedVisi} "</div>
                    <div className="w-full flex">
                        <PrimaryBtn loading={misiKdhState.loading} 
                            onClick={()=> openModalAction()}>
                            <PlusCircleIcon className="w-5 h-5" />
                            Tambah Misi
                        </PrimaryBtn>
                    </div>
                    <TableSection getDataAction={getDataTable} pagination={misiKdhState.pagination}>
                        <MyTable>
                            <TableHeader>
                            <tr>
                                <th scope="col" className="px-4 py-3 w-[3%]">No.</th>
                                <th scope="col" className="px-4 py-3">Misi</th>
                            ...
                            <TableBody>
                                {
                                    ...
                                    (misiKdhState.list.length > 0 ? 
                                        misiKdhState.list.map((item, key) => (
                                        <tr key={item.id} className="border-b dark:border-gray-700 odd:bg-gray-100 dark:odd:bg-gray-900">
                                            ...
                                            <td className="px-4 py-3">
                                                <Link to={`/perencanaan/kdh/pohonkinerja/tujuan?visiId=${selectedVisiId}&misiId=${item.id}`} className="hover:text-blue-500 hover:font-bold" >
                                                    {item.misi}
                                                </Link>
                                            </td>
                    ...
                    <MyModal  ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        <div className="flex flex-col w-full p-4">
                            <MyInput id="visiId" name="visiId" label="Visi"
                                value={selectedVisi} disabled />
                            ...
                            />
                            <MyTextarea id="misi" name="misi" label="Misi" placeholder='Inputkan misi...' 
                                value={formik.values.misi} onChange={formik.handleChange} onBlur={formik.handleBlur}
                                error={(formik.errors.misi && formik.touched.misi) ? formik.errors.misi : ""}
                            />
                        ...
                        <div className="mt-5 sm:mt-6 flex justify-center">
                            <PrimaryBtn onClick={()=> simpanData()} loading={misiKdhState.loading} >
                                Simpan Data
                            </PrimaryBtn>
                        </div>
                    </MyModal>
