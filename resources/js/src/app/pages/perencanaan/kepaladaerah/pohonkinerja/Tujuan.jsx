...
import { getListTujuanKdh, createTujuanKdh, updateTujuanKdh, deleteTujuanKdh } from '@/redux/ducks/tujuankdh/action'
import { getMisiKdh } from '@/redux/ducks/misikdh/action'
...
const Tujuan = () => {
    ...
    const [selectedMisi, setSelectedMisi] = React.useState("Misi")
    const [editId, setEditId] = React.useState("")
    const selectedVisiId = searchParams.get("visiId")
    const selectedMisiId = searchParams.get("misiId")
    React.useEffect(() => { initFlowbite() },[tujuanKdhState])
    const getDataTable = async (page = 1, per_page = 10, search = "") => {
        const response = await dispatch(getListTujuanKdh({ page, per_page, search, misi_id: selectedMisiId ?? "" }))
    ...
    React.useEffect(() => { misiKdhState.data ? setSelectedMisi(misiKdhState.data.misi) : null },[misiKdhState.data])
    React.useEffect(() => {

        selectedMisiId ? 
        dispatch( getMisiKdh(selectedMisiId ?? "") ).then((result) => {
    ...
    const simpanData= async ()=> {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            const payload = {
                pohon_kinerja_misi_id: selectedMisiId,
                order: form.order,
                tujuan: form.tujuan,
    ...
    const deleteAction = (id) => {
        ...
        confirmButtonText: 'Yes!'
            ...
            if (result.isConfirmed) {
                const response = await dispatch(deleteTujuanKdh(id))
    ...
    return (
        <Layout loading={misiKdhState.loading}>
            <HeaderPohonKinerja />
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-5 px-4 w-full flex min-h-[35rem]">
                ...
                <div className="flex flex-col w-full mt-14 p-3 border border-teal-500/30 dark:border-gray-600 rounded-lg">
                    <div className="w-full text-center text-lg text-teal-500 dark:text-white font-bold mb-3">TUJUAN KEPALA DAERAH</div>
                    <div className="w-full text-center text-lg dark:text-white italic mb-3">Misi : " {selectedMisi} "</div>
                    <div className="w-full flex">
                        <PrimaryBtn loading={tujuanKdhState.loading} onClick={() => openModalAction()} >
                            <PlusCircleIcon className="w-5 h-5" />
                            Tambah Tujuan
                        </PrimaryBtn>
                    </div>
                    <TableSection getDataAction={getDataTable} pagination={tujuanKdhState.pagination}>
                        <MyTable>
                            <TableHeader>
                            <tr>
                            ...
                            <TableBody>
                                {
                                    ...
                                    (tujuanKdhState.list.length > 0 ? 
                                        tujuanKdhState.list.map((item, key) => (
                                            <tr key={item.id} className="border-b dark:border-gray-700 odd:bg-gray-100 dark:odd:bg-gray-900">
                                                ...
                                                <td className="px-4 py-3">
                                                    <Link to={`/perencanaan/kdh/pohonkinerja/sasaran?visiId=${selectedVisiId}&misiId=${selectedMisiId}&tujuanId=${item.id}`} className="hover:text-blue-500 hover:font-bold" >
                                                        {item.tujuan}
                                                    </Link>
                                                </td>
                                                ...
                                                <td className="px-4 py-3 flex items-center justify-end">
                                                    ...
                                                    <div id={`toggle-btn${key}`}
                                                        ...
                                                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                            ...
                                                            <li>
                                                                <a href="#" onClick={() => navigate(`/perencanaan/kdh/pohonkinerja/indikator?visiId=${selectedVisiId}&misiId=${selectedMisiId}&tujuanId=${item.id}`)}
                    ...
                    <MyModal ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        <div className="flex flex-col w-full p-4">
                            <MyInput id="misiId" name="misiId" label="Misi"
                                value={selectedMisi} disabled />
                            ...
                            />
                            <MyTextarea id="tujuan" name="tujuan" label="Tujuan" placeholder='Inputkan tujuan...' 
                                value={formik.values.tujuan} onChange={formik.handleChange} onBlur={formik.handleBlur}
                        ...
                        <div className="mt-5 sm:mt-6 flex justify-center">
                            <PrimaryBtn onClick={()=> simpanData()} loading={tujuanKdhState.loading} >
                                Simpan Data
                            </PrimaryBtn>
                        </div>
                    </MyModal>
