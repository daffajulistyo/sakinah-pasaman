...
import { getListRenjaOpd, createRenjaOpd } from '@/redux/ducks/renjaopd/action'
...
const RenjaForm = () => {
    ...
    React.useEffect(() => {
        if(type !== "murni" && type !== "perubahan"){
            navigate('/perencanaan/opd/renja')
        }
        setFormTitle("FORM INPUT TARGET RENJA "+type.toUpperCase())
    },[type])
    React.useEffect(() => {
        initFlowbite()
    },[renjaOpdState.list]) 
    ...
    const getDataTable = async () => {
        ...
        const response = await dispatch(getListRenjaOpd({tahun, murni}))
    }
    const tableHeader = () => (
        <tr>
    ...
    const renderTable = () => {
        return renjaOpdState.list.length > 0 ? renjaOpdState.list.map((item, x) => (
            <>
            <tr key={x} className="border-b dark:border-gray-700">
                ...
                <td className="px-4 py-3 border text-right">
                    {
                        item.indikator_sasaran.length > 0 ? (item.indikator_sasaran[0].renja ?
                            (item.indikator_sasaran[0].renja.target) : "-"
                        ) : "-"
                    }
                </td>
                ...
                <td className="px-4 py-3 border flex justify-center">
                    ...
                    <div id={`toggle-btn${x}`}
                        ...
                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                            aria-labelledby={`btn-${x}`}>
                            {
                                ...
                                <li>
                                    <button onClick={() => inputTarget({
                                        ...
                                        target: item?.indikator_sasaran[0]?.renja?.target ?? ""
                                    })}
                            ...
                            <li>
                                <Link to={`/perencanaan/opd/renja/${period}/${type}/program?id=${item.id}`}
                                    className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                    <PencilSquareIcon className='w-5 h-5' />
                                    Input Program
                                </Link>
    ...
    const [targetRenja, setTargetRenja] = React.useState(0)
    const inputTarget = (data) => {
        setTargetRenja(data.target)
        setFormContent({
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/opd/renja/cetak`
            ...
            let filename = 'Rencana_Kerja_Perangkat_Daerah.pdf'; //    fallback filename
            if (disposition) {
        ...
        <Layout>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    <h1 className="text-center dark:text-white font-semibold text-lg">Rencana Kerja (RENJA) <br /> {period} <br/ >{type.toUpperCase()}</h1>
                    <div className="flex">
                        <PrimaryLinkBtn to={`/perencanaan/opd/renja`}>
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        !renjaOpdState.loading ? renderTable() :
                        <tr className="border-b dark:border-gray-700">
                    ...
                    <MyModal  ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        <div className="flex flex-col w-full p-4">
                            ...
                            <MyInput 
                                ...
                                value={targetRenja}
                                onChange={(e) => setTargetRenja(e.target.value)}
                            />
                        </div>
