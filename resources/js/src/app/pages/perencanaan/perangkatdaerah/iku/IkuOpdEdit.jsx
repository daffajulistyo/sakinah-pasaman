...
import { getIndikatorOpd } from '@/redux/ducks/indikatoropd/actions'
import { updateIkuOpd } from '@/redux/ducks/ikuopd/action'
...
const IkuOpdEdit = () => {
    ...
    React.useEffect(() => {
        setIkuData(null)
        if(indikatorOpdState.data?.id) setIkuData(indikatorOpdState.data)
    },[indikatorOpdState.data?.id])
    React.useEffect(() => {
        setForm()
    },[ikuData])
    const getIndikator = async () => {
        const response = await dispatch( getIndikatorOpd(id) )
        if(response.error !== null){
            Swal.fire({
                ...
                if(result.isConfirmed) navigate('/perencanaan/opd/iku')
            })
        }
    }

    const setForm = () => {
        if(indikatorOpdState.data !== null){
    ...
    const onSimpan = async () => {
        if (formik.isValid && formula !== "") {
            ...
            let response = null
            response = await dispatch(updateIkuOpd(id, payload))
    ...
    return (
        <Layout loading={indikatorOpdState.loading}>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                ...
                <div className="w-full flex justify-between py-5 px-2">
                    <PrimaryLinkBtn to='/perencanaan/opd/iku' >
