...
import { getListRealisasiRenaksiOpd, createRealisasiRenaksiOpd } from '@/redux/ducks/realisasirenaksiopd/action'
...
const RealisasiRenaksiOpd = () => {
    ...
    const getDataTable = (year = "") => {
        dispatch(getListRealisasiRenaksiOpd({ tahun: year != "" ? year : selectedYear }))        
    }
    ...
    const simpanData = async () => {
        let payload = {
            sasaran_opd_id: formContent.sasaranId,
            indikator_opd_id: formContent.indikatorId,
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/opd/realisasi/cetak?tahun=${selectedYear}`
            ...
            let filename = 'Realisasi_Rencana_Aksi_Perangkat_Daerah.pdf'; //    fallback filename
            if (disposition) {
        ...
        <Layout>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    <h1 className="text-center font-semibold text-lg dark:text-white">Realisasi Rencana Aksi Perangkat Daerah</h1>
                </div>
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        realisasiRenaksiOpdState.loading ?
                        <tr className="border-b dark:border-gray-700">
                            <td className='px-4 py-3 border text-center' colSpan="100%">
                                Loading...
                            </td>
                        </tr> :
                        realisasiRenaksiOpdState.list.map((item, key) => (
                            <>
                            <tr key={key} className="border-b dark:border-gray-700">
                                ...
                                <td className="px-4 py-3 border text-center align-top">
                                        ...
                                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <button onClick={() => inputRealisasi({
                                                    ...
                                                    indikatorId: item.indikator_sasaran[0]?.id ?? "",
                                                    indikator: item.indikator_sasaran[0]?.indikator ?? "",
                                                    rencana_aksi: item.indikator_sasaran[0]?.rencana_aksi
                                                })}
                                            ...
                                            <li>
                                                <Link to={`/pengukuran/opd/realisasirenaksi/langkah?sasaranid=${item.id}&indikatorid=${item.indikator_sasaran[0]?.id ?? "0"}&tahun=${selectedYear}`}
                                                    className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    <PencilSquareIcon className='w-4 h-4' />
                                                    Input Langkah
                                                </Link>
                                        ...
                                        if(x > 0){
                                            return (
                                                <tr key={x} className="border-b dark:border-gray-700">
                                                    ...
                                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi.hambatan ?? ""}</td>
                                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi.tindak_lanjut ?? ""}</td>
                                                    <td className="px-4 py-3 border text-center align-top">
                                                        ...
                                                        <div id={`toggle-btn${val.id}`}
                                                            ...
                                                            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                                ...
                                                                <li>
                                                                    <Link to={`/pengukuran/opd/realisasirenaksi/langkah?sasaranid=${item.id}&indikatorid=${val.id}&tahun=${selectedYear}`}
                                                                        className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                                        <PencilSquareIcon className='w-4 h-4' />
                                                                        Input Langkah
                                                                    </Link>
