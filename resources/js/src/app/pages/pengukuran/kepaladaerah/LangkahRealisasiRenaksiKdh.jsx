...
import { getListRealisasiRenaksiKdhLangkah, getListRealisasiRenaksiKdh, createRealisasiRenaksiKdhLangkah } from '@/redux/ducks/realisasirenaksikdh/action'

const LangkahRealisasiRenaksiKdh = () => {
    ...
    const getDataTable = () => {
        dispatch(getListRealisasiRenaksiKdhLangkah({
            sasaran_id: sasaranidActive,
            indikator_id: indikatoridActive,
            tahun: yearActive
        }))
    }
    React.useEffect(() => {
    ...
    React.useEffect(() => {
        if(realisasiRenaksiKdhState.list.length === 0){ 
            dispatch(getListRealisasiRenaksiKdh({ tahun: yearActive }))
        }
        else{
            let baseData = null
            let dataSasaran = realisasiRenaksiKdhState.list.find((item) => {
                return item.id === sasaranidActive
            })
            if(dataSasaran){
                let dataIndikator = dataSasaran.indikator.find((item) => {
    ...
    React.useEffect(() => { initFlowbite() }, [realisasiRenaksiKdhState.list_langkah])
    ...
    const renderTable = () => (
        realisasiRenaksiKdhState.list_langkah.length > 0 ? realisasiRenaksiKdhState.list_langkah.map((item, x) => (
    ...
    const simpanData= async ()=> {
        ...
        if (Object.keys(errors).length === 0) {
            ...
            payload.pohon_kinerja_sasaran_id = sasaranidActive
            payload.pohon_kinerja_indikator_id = indikatoridActive
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 px-3 w-full">
                <div className="dark:text-white">
                    <div className="flex flex-row items-center gap-3">
                        ...
                        <div className="lg:text-lg font-bold text-teal-500 dark:text-white">Pengukuran Kepala Daerah</div>
                    </div>
                </div>
            </div>
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    ...
                    <div className="flex justify-between gap-3">
                        <PrimaryLinkBtn to={`/pengukuran/kdh/realisasirenaksi`}>
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        !realisasiRenaksiKdhState.loading ? renderTable() :
                        <tr className="border-b dark:border-gray-700">
...
export default LangkahRealisasiRenaksiKdh