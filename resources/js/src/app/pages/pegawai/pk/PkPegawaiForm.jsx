...
import { createPkPegawai, getListPkPegawai } from '@/redux/ducks/pkpegawai/action'
...
const PkPegawaiForm = () => {
    ...
    const getDataTable = async () => {
        ...
        const response = await dispatch(getListPkPegawai({tahun, murni, eselon: "II"}))
    }
    ...
    const renderTable = () => {
        return pkPegawaiState.list.length > 0 ? pkPegawaiState.list.map((item, x) => (
            <>
                ...
                <td className="px-4 py-3 border flex justify-center">
                    ...
                    <div id={`toggle-btn${x}`}
                        ...
                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                            ...
                            <li>
                                <Link to={`/pegawai/pk/${period}/${type}/program?id=${item.id}`}
                                    className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                    <PencilSquareIcon className='w-5 h-5' />
                                    Input Program
                                </Link>
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/pegawai/perjanjiankinerja/cetak?tahun=${period}&murni=${type === 'murni' ? 'true' : 'false'}&eselon=II`
            ...
            let filename = `Perjanjian_Kinerja_${authState.biodata.name ?? 'noname'}.pdf`; //    fallback filename
            if (disposition) {
        ...
        <Layout>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    <h1 className="text-center font-semibold text-lg">Perjanjian Kinerja <br /> {period} <br/ >{type.toUpperCase()}</h1>
                    <div className="flex">
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        !pkPegawaiState.loading ? renderTable() :
                        <tr className="border-b dark:border-gray-700">
                    ...
                    <MyModal  ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        ...
                        <div className="mt-5 sm:mt-6 flex justify-center">
                            <PrimaryBtn loading={pkPegawaiState.loading} onClick={() => simpanData()} >
                                Simpan Data
                            </PrimaryBtn>
                        </div>
                    </MyModal>
