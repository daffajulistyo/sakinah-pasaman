...
import { getListRealisasiRenaksiPegawai, createRealisasiRenaksiPegawai } from '@/redux/ducks/realisasirenaksipegawai/action'
import axios from 'axios'

const RealisasiRenaksiPegawai = () => {
    const realisasiRenaksiPegawaiState = useSelector((state) => state.realisasiRenaksiPegawaiState)
    ...
    const getDataTable = (year = "") => {
        dispatch(getListRealisasiRenaksiPegawai({ tahun: year != "" ? year : selectedYear }))        
    }
    ...
    React.useEffect(() => {
        initFlowbite()
    },[realisasiRenaksiPegawaiState.list])
    const tableHeader = () => (
        <>
            <tr>
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/pegawai/realisasi/cetak?tahun=${selectedYear}`
            ...
            let filename = `Realisasi_Rencana_Aksi_${selectedYear}_${authState.biodata.name}.pdf`; //    fallback filename
            if (disposition) {
        ...
        <Layout>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                <div className="block w-full p-4">
                    <h1 className="text-center font-semibold text-lg dark:text-white">Realisasi Rencana Aksi Pegawai</h1>
                </div>
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        realisasiRenaksiPegawaiState.loading ?
                        <tr className="border-b dark:border-gray-700">
                            <td className='px-4 py-3 border text-center' colSpan="100%">
                                Loading...
                            </td>
                        </tr> :
                        realisasiRenaksiPegawaiState.list.length == 0 ?
                        <tr className="border-b dark:border-gray-700">
                            <td className='px-4 py-3 border text-center' colSpan="100%">
                                No Data
                            </td>
                        </tr> :
                        realisasiRenaksiPegawaiState.list.map((item, key) => (
                            <>
                            <tr key={key} className="border-b dark:border-gray-700">
                                ...
                                <td className="px-4 py-3 border">{item.indikator_sasaran[0]?.indikator ?? "(belum ada data)"}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.target_tw1 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.realisasi_tw1 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.capaian_tw1 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.target_tw2 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.realisasi_tw2 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.capaian_tw2 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.target_tw3 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.realisasi_tw3 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.capaian_tw3 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.target_tw4 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.realisasi_tw4 ?? ""}</td>
                                <td className="px-4 py-3 border text-right">{item.indikator_sasaran[0].rencana_aksi?.capaian_tw4 ?? ""}</td>
                                ...
                                <td className="px-4 py-3 border text-center">
                                    ...
                                    <div id={`toggle-btn${key}`}
                                        ...
                                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <Link to={`/pegawai/realisasirenaksi/langkah?sasaranid=${item.id}&indikatorid=${item.indikator_sasaran[0]?.id ?? "0"}&tahun=${selectedYear}`}
                                                    className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    <PencilSquareIcon className='w-4 h-4' />
                                                    Input Langkah
                                                </Link>
                                                    ...
                                                    <td className="px-4 py-3 border text-center">
                                                        ...
                                                        <div id={`toggle-btn${val.id}`}
                                                            ...
                                                            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                                ...
                                                                <li>
                                                                    <Link to={`/pegawai/realisasirenaksi/langkah?sasaranid=${item.id}&indikatorid=${val.id}&tahun=${selectedYear}`}
                                                                        className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                                        <PencilSquareIcon className='w-4 h-4' />
                                                                        Input Langkah
                                                                    </Link>
                    ...
                    <MyModal  ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        ...
                        <div className="flex justify-center">
                            <PrimaryBtn loading={realisasiRenaksiPegawaiState.loading} onClick={() => simpanData()} >
                                Simpan Data
                            </PrimaryBtn>
                        </div>
                    </MyModal>
...
export default RealisasiRenaksiPegawai