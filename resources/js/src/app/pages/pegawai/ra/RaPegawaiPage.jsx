...
import { createRenaksiPegawai, getListRenaksiPegawai } from '@/redux/ducks/renaksipegawai/action'
...
const RaPegawaiPage = () => {
    ...
    const renaksiPegawaiState = useSelector((state) => state.renaksiPegawaiState)
    ...
    const inputTarget = (data) => {
        ...
        setTargettw1(data.rencana_aksi?.target_tw1 ?? 0)
        setTargettw2(data.rencana_aksi?.target_tw2 ?? 0)
        setTargettw3(data.rencana_aksi?.target_tw3 ?? 0)
        setTargettw4(data.rencana_aksi?.target_tw4 ?? 0)
        setOpenModal(true)
    }
    const getDataTable = (tahun = "") => {
        dispatch(getListRenaksiPegawai({tahun: tahun !== "" ? tahun : selectedYear }))
    }
    React.useEffect(() => {
        getDataTable()
    },[])
    React.useEffect(() => {
        initFlowbite()
    },[renaksiPegawaiState.list]) //renaksiPegawaiState.list
    ...
    const download = async () => {
        try {
            ...
            const apiUrl = `${BASE_HOST_URL}/v1/pegawai/aksi/cetak?tahun=${selectedYear}`
            ...
            let filename = `Rencana_Aksi_${selectedYear}_${authState.biodata.name}.pdf`; //    fallback filename
            if (disposition) {
        ...
        <Layout>
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 px-3 w-full">
                <div className="dark:text-white">
                    <div className="flex flex-row items-center gap-3">
                        ...
                        <div className="lg:text-lg font-bold text-teal-500 dark:text-white">Perencanaan Rencana Aksi Pegawai</div>
                    </div>
                </div>
            </div>
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        renaksiPegawaiState.loading ? 
                        <tr className="border-b dark:border-gray-700">
                            <td className="px-4 py-3 border text-center" colSpan="100%">Loading...</td>
                        </tr>
                        :
                        (
                        renaksiPegawaiState.list.length > 0 ?
                        renaksiPegawaiState.list.map((item, key) => (
                            <>
                                        ...
                                        <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                            ...
                                            <li>
                                                <button onClick={() => inputTarget({
                                                    ...
                                                    rencana_aksi: item.indikator_sasaran[0]?.rencana_aksi ?? null
                                                })}
                                            ...
                                            <li>
                                                <Link to={`/pegawai/ra/langkah?sasaranid=${item.id}&indikatorid=${item.indikator_sasaran[0]?.id ?? "0"}&tahun=${selectedYear}`}
                                                    className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                    <PencilSquareIcon className='w-4 h-4' />
                                                    Input Langkah
                                                </Link>
                                                            ...
                                                            <ul className="py-1 text-sm text-gray-700 dark:text-gray-200"
                                                                ...
                                                                <li>
                                                                    <button onClick={() => inputTarget({
                                                                        ...
                                                                        rencana_aksi: val.rencana_aksi ?? null
                                                                    })}
                                                                        className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                                        <PencilSquareIcon className='w-4 h-4' />
                                                                        Input Target
                                                                ...
                                                                <li>
                                                                    <Link to={`/pegawai/ra/langkah?sasaranid=${item.id}&indikatorid=${val.id}&tahun=${selectedYear}`}
                                                                        className="flex gap-1 py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">
                                                                        <PencilSquareIcon className='w-4 h-4' />
                                                                        Input Langkah
                                                                    </Link>
                    ...
                    <MyModal  ModalTitle={formTitle} openModal={openModal} setOpenModal={setOpenModal} >
                        ...
                        <div className="mt-5 sm:mt-6 flex justify-center">
                            <PrimaryBtn loading={renaksiPegawaiState.loading} onClick={() => simpanData()} >
                                Simpan Data
                            </PrimaryBtn>
                        </div>
                    </MyModal>
