...
import { getListRenaksiPegawaiLangkah, createRenaksiPegawaiLangkah, updateRenaksiPegawaiLangkah, deleteRenaksiPegawaiLangkah, getListRenaksiPegawai } from '@/redux/ducks/renaksipegawai/action'

const LangkahRaPegawai = () => {
    ...
    const getDataTable = () => {
        dispatch(getListRenaksiPegawaiLangkah({
    ...
    React.useEffect(() => {
        if(renaksiPegawaiState.list.length === 0){ 
            dispatch(getListRenaksiPegawai({ tahun: yearActive })) 
        }
        else{
            let baseData = null
            console.log(renaksiPegawaiState.list);
            
            let dataSasaran = renaksiPegawaiState.list.find((item) => {
    ...
    }, [renaksiPegawaiState.list])
    React.useEffect(() => { initFlowbite() }, [renaksiPegawaiState.list_langkah]) // renaksiKdhState.list_langkah
    ...
    const renderTable = () => (
        renaksiPegawaiState.list_langkah.length > 0 ? renaksiPegawaiState.list_langkah.map((item, x) => (
    ...
    const deleteAction = (id) => {
        ...
        confirmButtonText: 'Yes!'
            ...
            if (result.isConfirmed) {
                const response = await dispatch(deleteRenaksiPegawaiLangkah(id))
        ...
        <Layout>
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 px-3 w-full">
                <div className="dark:text-white">
                    <div className="flex flex-row items-center gap-3">
                        <div>
                            <img src={GoodNotes} alt="Perencanaan Pegawai" className="object-contain" />
                        </div>
            ...
            <div className="bg-white h-auto dark:bg-gray-800 rounded-lg drop-shadow-xl py-1 sm:px-3 px-1 w-full flex flex-col lg:min-h-[35rem]">
                ...
                <div className="block w-full p-4">
                    ...
                    {
                        !renaksiPegawaiState.loading ? renderTable() :
                        <tr className="border-b dark:border-gray-700">
...
export default LangkahRaPegawai