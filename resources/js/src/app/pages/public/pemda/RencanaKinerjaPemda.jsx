...
import { getPublicVisiPemda, getPublicRencanaKinerjaPemda } from '@/redux/ducks/public/action'
import { StaticTable } from '@/app/components/Table'

const RencanaKinerjaPemda = () => {
    ...
    React.useEffect(() => {
        if(selectedYear !== ''){
            dispatch(getPublicRencanaKinerjaPemda({ tahun: selectedYear, murni:true }))
        }
    },[selectedYear])

    const renderTable = () => {
        return publicDataState.data_rencanakinerja_pemda.length > 0 ? publicDataState.data_rencanakinerja_pemda.map((item, x) => (
            <>
        ...
        <PublicLayout loading={publicDataState.loading}>
            <div className="w-full px-4 md:py-6 py-2">
                <div className="w-full mx-auto max-w-screen-2xl">
                    <h1 className="font-bold md:text-2xl sm:text-xl text-lg text-primaryWebColor">RENCANA KINERJA PEMERINTAH PROVINSI SUMATERA BARAT</h1>
                </div>
            </div>
            <div className="w-full max-w-screen-2xl min-h-screen bg-white mx-auto border p-4 rounded-lg">
                <h1 className="text-xl font-bold text-center mb-3"> RENCANA KINERJA GUBERNUR {selectedYear !== '' ? ('TAHUN '+selectedYear) : ''}</h1>
                ...
                <div className="w-full">
                    ...
                    {
                        (publicDataState.data_rencanakinerja_pemda === null || selectedYear === '') ? 
                        <tr className="border-b dark:border-gray-700">
...
export default RencanaKinerjaPemda