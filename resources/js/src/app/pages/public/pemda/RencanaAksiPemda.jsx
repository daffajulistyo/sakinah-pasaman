...
import { getPublicVisiPemda, getPublicRenaksiPemda } from '@/redux/ducks/public/action'
import { StaticTable } from '@/app/components/Table'
import { RupiahFormatter } from '@/helper/common'

const RencanaAksiPemda = () => {
    ...
    React.useEffect(() => {
        if(selectedYear !== ''){
            dispatch(getPublicRenaksiPemda({ tahun: selectedYear, murni:true }))
        }
    },[selectedYear])
    const renderTable = () => {
        return publicDataState.data_renaksi_pemda.map((item, key) => (
            <>
        ...
        <PublicLayout loading={publicDataState.loading}>
            ...
            <div className="w-full max-w-screen-2xl min-h-screen bg-white mx-auto border p-4 rounded-lg">
                ...
                <div className="w-full">
                    ...
                    {
                        (publicDataState.data_renaksi_pemda === null || selectedYear === '') ? 
...
export default RencanaAksiPemda