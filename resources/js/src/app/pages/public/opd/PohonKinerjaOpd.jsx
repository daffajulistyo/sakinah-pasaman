...
import { getListPublicPohonKinerjaOpd, getPublicVisiPemda, getPublicDaftarOpd } from '@/redux/ducks/public/action'
...
const PohonKinerjaOpd = () => {
    ...
    React.useEffect(() => {
        if(selectedOpd !== null)
        {
            ...
            dispatch(getListPublicPohonKinerjaOpd(payload))
        }
    },[selectedOpd])
    ...
    },[publicDataState.data_pohonkinerja_opd])
    ...
    const chartData = () => {
        let data = null
        try {
            if(publicDataState.data_pohonkinerja_opd !== null){
                const pkOpd = publicDataState.data_pohonkinerja_opd
                ...
                data = {
                    ...
                    name: renderMisi("Pohon Kinerja <br />"+selectedOpd.label, "1", "asdasd-asdasdasd-asdasd"),
                    children: tujuan
                }
            }
        ...
        <PublicLayout loading={publicDataState.loading}>
            <div className="w-full px-4 md:py-6 py-2">
                <div className="w-full mx-auto max-w-screen-2xl">
                    <h1 className="font-bold md:text-2xl sm:text-xl text-lg text-primaryWebColor">POHON KINERJA PERANGKAT DAERAH</h1>
                </div>
            </div>
...
const renderMisi = (misi, urutan, id) => {
    return `
    <a href="/perencanaan/opd/pohonkinerja/tujuan" style="display : flex; flex; flex-direction: column; background-color: #D6C0B3; color: white; padding: 16px; border-radius: 35px; 
...
const renderTujuan = (tujuan, id, indikator) => {
    ...
    return `
    <a href="/perencanaan/opd/pohonkinerja/sasaran?id=${id}" style="display : flex; flex; flex-direction: column; background-color: #AB886D; color: white; padding: 4px; border-radius: 35px; width: 100%; height: 100%; justify-content: center; align-items: center; text-align: center;">
        Tujuan : <br />
...
const renderSasaran = (sasaran, urutan, tujuanId, sasaranId, indikator) => {
    ...
    return `
    <a href="/perencanaan/opd/pohonkinerja/indikator?tujuanid=${tujuanId}&sasaranid=${sasaranId}" style="display: flex; flex-direction: column; background-color: #493628; color: white; padding: 35px; border-radius: 35px; width: 100%; height: 100%; justify-content: center; align-items: center; text-align: center;">
        Sasaran ${urutan} : <br />
