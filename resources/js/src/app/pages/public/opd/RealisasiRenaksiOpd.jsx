...
import { getPublicVisiPemda, getPublicDaftarOpd, getPublicRealisasiRenaksiOpd } from '@/redux/ducks/public/action'
...
const RealisasiRenaksiOpd = () => {
    ...
    React.useEffect(() => {
        if(selectedOpd !== null && selectedYear !== '')
        {
            ...
            dispatch(getPublicRealisasiRenaksiOpd(payload))
        }
    ...
    const renderTable = () => {
        return publicDataState.data_realisasirenaksi_opd.length > 0 ?
        publicDataState.data_realisasirenaksi_opd.map((item, key) => (
            <>
            <tr key={key} className="border-b dark:border-gray-700">
                <td 
                    className="px-4 py-3 border text-right align-top"
                ...
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.realisasi_tw1 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.capaian_tw1 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.target_tw2 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.realisasi_tw2 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.capaian_tw2 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.target_tw3 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.realisasi_tw3 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.capaian_tw3 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.target_tw4 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.realisasi_tw4 ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.capaian_tw4 ?? ""}</td>
                ...
                <td className="px-4 py-3 border text-right align-top nowrap" rowSpan={item.indikator_sasaran.length > 0 ? item.indikator_sasaran.length : 1}>{
                ...
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.hambatan ?? ""}</td>
                <td className="px-4 py-3 border text-right align-top">{item.indikator_sasaran[0]?.rencana_aksi?.tindak_lanjut ?? ""}</td>
                        ...
                        if(x > 0){
                            return (
                                <tr key={x} className="border-b dark:border-gray-700">
                                    ...
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.realisasi_tw1 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.capaian_tw1 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.target_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.realisasi_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.capaian_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.target_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.realisasi_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.capaian_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.target_tw4 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.realisasi_tw4 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{val.rencana_aksi?.capaian_tw4 ?? ""}</td>
                                    <td className="px-4 py-3 border align-top">
                                    {
                                    ...
                                    <td className="px-4 py-3 border text-right align-top">{item.rencana_aksi?.hambatan ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.rencana_aksi?.tindak_lanjut ?? ""}</td>
    ...
    const langkahTableHeader = () => (
        <>
            <tr>
                ...
                <th scope="col" className="px-4 py-3 border text-center" colSpan="3">TW I</th>
                <th scope="col" className="px-4 py-3 border text-center" colSpan="3">TW II</th>
                <th scope="col" className="px-4 py-3 border text-center" colSpan="3">TW III</th>
                <th scope="col" className="px-4 py-3 border text-center" colSpan="3">TW IV</th>
            </tr>
            <tr>
        ...
        <PublicLayout loading={publicDataState.loading}>
            <div className="w-full px-4 md:py-6 py-2">
                <div className="w-full mx-auto max-w-screen-2xl">
                    <h1 className="font-bold md:text-2xl sm:text-xl text-lg text-primaryWebColor">REALISASI RENCANA AKSI PERANGKAT DAERAH</h1>
                </div>
            </div>
            <div className="w-full max-w-screen-2xl min-h-screen bg-white mx-auto border p-4 rounded-lg">
                ...
                <div className="w-full">
                    ...
                    {
                        (publicDataState.data_realisasirenaksi_opd === null || selectedOpd === null || selectedYear === '') ? 
                        <tr className="border-b dark:border-gray-700">
            ...
            <MyModal size='lg' ModalTitle={"Realisasi Rencana Aksi"} openModal={openModal} setOpenModal={setOpenModal} >
                    ...
                    <StaticTable header={langkahTableHeader()}>
                        {
                            ...
                            viewLangkah.map((item, x) => (
                                <tr key={x} className="border-b dark:border-gray-700">
                                    ...
                                    <td className="px-4 py-3 border text-right align-top">{item.realisasi_tw1 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.capaian_tw1 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.target_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.realisasi_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.capaian_tw2 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.target_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.realisasi_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.capaian_tw3 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.target_tw4 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.realisasi_tw4 ?? ""}</td>
                                    <td className="px-4 py-3 border text-right align-top">{item.capaian_tw4 ?? ""}</td>
                                </tr>
                            ))
                            :
                            <tr className="border-b dark:border-gray-700">
...
export default RealisasiRenaksiOpd